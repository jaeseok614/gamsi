...
async function createTempEmployee(request, adminCookie, label) {
  ...
  const email = `groupware-${label}-${stamp}@gamsi.kr`;
...
test("그룹웨어 공지 댓글 첨부 검색이 동작한다", async ({ page, request, baseURL }) => {
  ...
  const outOfTeamEmployee = await createTempEmployee(request, adminCookie, "out-of-team");
  const outOfTeamCookie = await loginApi(request, outOfTeamEmployee.email);
  const blockedTeamNoticeDownload = await request.get(`${baseURL}/api/groupware/announcement-attachments/${teamOnlyAttachment.id}`, {
    headers: {
      cookie: outOfTeamCookie
    }
  });
  ...
  const boardSearchResult = page.locator("a.notification-card").filter({ hasText: boardTitle }).first();
  ...
  await expect(page.locator(".groupware-operation-card").filter({ hasText: "게시물 등록" }).first()).toBeVisible();
...
test("그룹웨어 메모 실적 급여명세가 동작한다", async ({ page, request, baseURL }) => {
  ...
  await expect(page.getByText(memoText).first()).toBeVisible();
...
test("권한별 그룹웨어 회귀와 전자결재 고도화 흐름이 동작한다", async ({ page, request, baseURL }) => {
  ...
  await addSessionCookie(page, adminCookie, baseURL);
  await page.goto(`/dashboard?view=groupware&groupwareTab=operations&groupwareOperationAction=document_request.delegated_reviewed`, {
    waitUntil: "domcontentloaded"
  });
  await expect(page.locator(".groupware-operation-card").filter({ hasText: "대리 결재 처리" }).first()).toBeVisible();
});
